...
console.log(getAllAmount(newClient.amounts));
//console.log((getAllAmount(clients[0].amounts)));

// 4.
let getAverageAmount = (totalArray) => {
  return (getAllAmount(totalArray) / totalArray.length).toFixed();
};
...
let showClients = (clients) => {
  clients.forEach((client) => {
    let message =
      ...
      getAverageAmount(client.amounts) +
      " рублей. " +
...
let whoSpentMore = (clients) => {
  ...
  if (clientWithMaxSpent) {
    ...
    console.log(
      "Больше всех потратил " + name + " " + " Сумма покупок: " + max
    );
  } else {
